...
# Page configuration
st.set_page_config(layout="wide", page_title="InField AI vs Traditional Workflow")

# Header
st.title("🧠 InField AI – System of Work vs Traditional CRM Workflow")

st.markdown("""
### 💡 Goal:
Create a self-explanatory, interactive product demo showing how **InField AI** (built on MCP) outperforms traditional workflows.
""")
...
# --- Right Panel: InField AI (MCP) --- #
with col2:
    st.header("🤖 InField AI – System of Work (MCP)")
    st.markdown("---")

    with st.expander("1️⃣ Ask a Natural Language Question"):
        query = st.text_input("You:", "Who should I meet in Bangalore South today?")
        if query:
    ...
    with st.expander("3️⃣ Personalized Pitch Generation"):
        pitch_request = st.text_input("You:", "Suggest pitch for Dr. Mehta")
        if pitch_request:
